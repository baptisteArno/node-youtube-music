...
import {parseAlbumSearchResult} from './parsers'


// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const parseAlbumsSearchBody = (body: any): AlbumPreview[] =>{
  ...
  contents.forEach((content: any)=> {
    try {
      const album = parseAlbumSearchResult(content);
      if (album) {
        results.push(album);
      }
    }
